...
describe("User service tests", () => {
  ...
  describe("Create a user with disabled status", () => {
    test("should return an error if the email is already present", async () => {
      ...
      expect(userService.createUserWithDisabledStatus("test@test.com", "1234", "JJ", 1)).rejects.toEqual(
    ...
    test("should create a new user with the required fields", async () => {
      ...
      const res = await userService.createUserWithDisabledStatus(newUser.email, newUser.password, newUser.name, newUser.clientID as number);
  ...
  describe("Enroll a new user", () => {
    ...
    test("should return an error if the status code is not 200", async () => {
      ...
      expect(userService.enrollUser(sampleUser.email, sampleUser.password, sampleUser.name, clientID, sampleUser.userData, "")).rejects.toEqual(
        new ApiError(httpStatus.BAD_REQUEST, 'Orion request failed')
      )
    })
    ...
    test("should return an error if the summary action is Not Passed", async () => {
      ...
      expect(userService.enrollUser(sampleUser.email, sampleUser.password, sampleUser.name, clientID, sampleUser.userData, "")).rejects.toEqual(
        new ApiError(httpStatus.BAD_REQUEST, orionRes.result.summary.message)
      )
    ...
    test("should return an error if the user already exists with the particular email", async () => {
      ...
      jest.spyOn(orionRequest, "orionRequest").mockResolvedValue(orionRes);
      expect(userService.enrollUser(userEmail, sampleUser.password, sampleUser.name, clientID, sampleUser.userData, "")).rejects.toEqual(
        new ApiError(
          httpStatus.BAD_REQUEST,
          `User already exists with a the email ${userEmail}`
        )
      )
    })
    ...
    test("should create a new user", async () => {
      ...
      const res = await userService.enrollUser(sampleUser.email, sampleUser.password, sampleUser.name, clientID, sampleUser.userData, sampleUser.status);
  ...
  describe("Get all logs", () => {
    ...
    test("should return the user details if the user is present", async () => {
      jest.spyOn(prisma.enrollment, "findMany").mockResolvedValueOnce(mockLogs as unknown as Enrollment[])
