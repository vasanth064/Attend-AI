...
import { Session, useGetSessionsForMachineQuery } from "@/redux/machine/machineApiSlice";
          ...
          {data === undefined ?
            [1, 1, 1, 1, 1].map((_, index) => (
              <TableRow key={index}>
                ...
                <TableCell><Skeleton className="h-[30px]" /></TableCell>
                <TableCell><Skeleton className="h-[30px]" /></TableCell>
                <TableCell className="w-[100px]"><Skeleton className="h-[30px]" /></TableCell>
              </TableRow>
            ))
            : data.map((d: Session) => (
              <TableRow key={d.id}>
                ...
                <TableCell>
                  <Button onClick={(_: React.MouseEvent<HTMLElement>) => {
                    dispatch(setSession({ id: d.id, name: d.name, startDateTime: d.startDateTime, endDateTime: d.endDateTime }));
